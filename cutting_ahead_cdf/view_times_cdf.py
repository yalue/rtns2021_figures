...
def get_line_styles():
    ...
    # (These are in [on-time, off-time, ...] pairs.
    dashes_options = [
        [1, 0],
        [1, 1, 1, 1],
        [3, 1, 3, 1],
...
def plot_scenario(plugins, name, times_key):
    ...
    # Paper-specific modification: swap the last two plugins' order so that
    # the legend keys are in the same order as the curves from left to right.
    plugins[2], plugins[1] = plugins[1], plugins[2]
    ...
    for i in range(len(cdfs)):
        axes.plot(cdfs[i][0], cdfs[i][1], label=labels[i], lw=2,
            **next(style_cycler))
